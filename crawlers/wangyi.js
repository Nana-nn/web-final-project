...
var author_format = "$('.icon').eq(0).attr(\"alt\")";
var source_format = "$('.post_info').children(':first').text()";
var desc_format = " $('meta[name=\"description\"]').eq(0).attr(\"content\")";
var content_format = "$('div.post_body').text()";
var url_reg = /\/news\/article\/([a-zA-Z0-9]{16}).html/;
...
function newsGet(myURL) { //读取新闻页面
    request(myURL, function(err, res, body) { //读取新闻页面
        ...
        // 新闻摘要
        fetch.description = eval(desc_format).replace("\r\n", ""); //摘要    
