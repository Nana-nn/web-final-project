...
var seedURL = 'https://www.sina.com.cn/';
...
var url_reg = /\/(\d{4})-(\d{2})-(\d{2})\/doc-([a-zA-Z0-9]{15}).shtml/;
...
request(seedURL, function(err, res, body) { 
    ...
    seedurl_news.each(function(i, e) { //遍历种子页面里所有的a链接
        ...
        try {
            ...
            var href = "";
            href = $(e).attr("href");
            if (typeof(href) == "undefined") {  // 有的地址undefined
                return true;
            }
...
function newsGet(myURL) { //读取新闻页面
    request(myURL, function(err, res, body) { //读取新闻页面
        ...
        if( fetch.content==""){
            fetch.content =  eval("$('#article_content').text()").replace(/[\r\n\s]/g, "");
        }
        ...
        fetch.source = eval(source_format);
         ...
         if (author_format == "") fetch.author = source_name; //eval(author_format);  //作者
             ...
             fetch.author = eval(author_format).replace("责任编辑：", "");
             if(fetch.author=='' ||fetch.author==null){
                 fetch.author=fetch.source;
             }
         }
        ...
        if (fetch.description == "") {
            fetch.description = eval("$('meta[property=\"og:description\"]').eq(0).attr(\"content\")");
        } 
